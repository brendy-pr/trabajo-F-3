...
def calcular_abc(productos: pd.DataFrame, stock: pd.DataFrame) -> pd.DataFrame:
    ...
    ventas = stock[["codigo_barras", "salidas"]] if not stock.empty else pd.DataFrame()
    abc = productos.merge(ventas, on="codigo_barras", how="left")
    abc["unidades_vendidas"] = abc["salidas"].fillna(0).astype(float)
    abc["valor_ventas"] = abc["unidades_vendidas"] * abc["precio_venta"].fillna(0).astype(float)
...
def pagina_dashboard():
    ...
    stock_total = int(productos["stock_actual"].fillna(0).sum()) if not productos.empty else 0
    valor_inventario = (
        productos["stock_actual"].fillna(0) * productos["precio_compra"].fillna(0)
    ).sum() if not productos.empty else 0
    venta_estimada = sugerencias["demanda_pronosticada"].fillna(0).sum() if not sugerencias.empty else 0
